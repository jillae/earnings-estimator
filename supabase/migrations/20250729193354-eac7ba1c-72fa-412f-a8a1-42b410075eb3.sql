...
ADD COLUMN IF NOT EXISTS leasing_tariffs jsonb DEFAULT '{}',
ADD COLUMN IF NOT EXISTS leasing_min integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS leasing_max integer DEFAULT 120,
ADD COLUMN IF NOT EXISTS credits_per_treatment integer DEFAULT 1,
...
-- Lägg till unik begränsning på name
ALTER TABLE public.machines ADD CONSTRAINT unique_machine_name UNIQUE (name);

-- Lägg till RLS policies för admin-åtkomst (ta bort befintlig först)
...
-- Lägg till exempel-data för befintliga maskiner
...
ON CONFLICT (name) DO UPDATE SET
  ...
  leasing_tariffs = EXCLUDED.leasing_tariffs,
